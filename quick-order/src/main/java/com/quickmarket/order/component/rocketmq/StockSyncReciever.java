...
public class StockSyncReciever implements RocketMQListener<String> {
    ...
    public void onMessage(String message) {
         ...
         */
        if(redisOpsUtil.hasKey(RedisKeyPrefixConst.NORMAL_STOCK_REFRESHED_MESSAGE_PREFIX + promotionId)){
            log.info("start sync mysql stock to redis");
            //todo 同步一下库存到缓存当中
            Integer stock = stockManageFeignApi.selectStock(productID,promotionId).getData();
            if(stock > 0){
                //重置库存
                redisOpsUtil.set(RedisKeyPrefixConst.NORMAL_STOCK_CACHE_PREFIX + productID,stock);
                //删除同步标记
                redisOpsUtil.delete(RedisKeyPrefixConst.NORMAL_STOCK_REFRESHED_MESSAGE_PREFIX + promotionId);
            }
        }
    }
