...
import com.quickmarket.order.component.CancelOrderSender;
...
import com.quickmarket.order.feignapi.ums.UmsCouponFeignApi;
import com.quickmarket.order.feignapi.ums.UmsMemberFeignApi;
import com.quickmarket.mbg.mapper.*;
import com.quickmarket.mbg.model.*;
import com.quickmarket.order.service.OmsCartItemService;
import com.quickmarket.order.service.OmsPortalOrderService;
import com.quickmarket.order.service.RedisService;
...
public class OmsPortalOrderServiceImpl implements OmsPortalOrderService {
    ...
    @Autowired
    private OrderMessageSender orderMessageSenderNomal;
    ...
    public CommonResult<List<OmsOrderDetail>> findMemberOrderList(Integer pageSize, Integer pageNum, Long memberId, Integer status) {
        ...
        List<OmsOrderDetail> omsOrderDetailList = portalOrderDao.findMemberOrderList(memberId,status);
        PageInfo<OmsOrderDetail> info = new PageInfo(omsOrderDetailList);
        return CommonResult.success(omsOrderDetailList, "sucess" ,(int)info.getTotal(), info.getPages());
    }
     ...
     */
    @Override
    public CommonResult getConfirmOrder(Long productId, Long skuId, Long memberId) throws BusinessException {
        // 【2】调用会员服务获取会员信息
        UmsMember member = umsMemberFeignApi.getMemberById().getData();

        // 【3】从产品服务获取产品信息
        PmsProductParam product = getProductInfo(productId);
        PmsSkuStock respmsSkuStock = null;
        // 通过商品和sku表获取信息
        if (skuId != null) {
            PmsSkuStock pmsSkuStock = new PmsSkuStock();
            pmsSkuStock.setProductId(productId);
            pmsSkuStock.setId(skuId);
            respmsSkuStock = skuStockMapper.getPmsSkuStock(pmsSkuStock);
        }
        ...
        promotionItem.setQuantity(1);
        Integer stock = respmsSkuStock.getStock();
    ...
    private ConfirmOrderResult.CalcAmount calcCartAmount(PmsProductParam product) {
        ...
        calcAmount.setFreightAmount(new BigDecimal(10));
        BigDecimal totalAmount = product.getPrice();
        BigDecimal promotionAmount = new BigDecimal("0");
        calcAmount.setTotalAmount(totalAmount);
        calcAmount.setPromotionAmount(promotionAmount);
        calcAmount.setPayAmount(product.getPrice());
        return calcAmount;
    }

    @Override
    public CommonResult<Map<String, Object>> generateOrderByQuick(OrderParam orderParam, Long memberId) throws BusinessException {
        // 获取商品信息，用户信息（会员等级），地址
        ...
        // 获取商品id，商品名称，商品图片，商品价格，商品品牌，商品货号
        ...
        orderItem.setPromotionName("欢迎购买");
        // 商品赠送积分，商品的成长值，商品优惠价金额，
        ...
        // 折扣金额，运费，促销金额，促销名称，
        order.setDiscountAmount(new BigDecimal(0));//折扣金额
        order.setFreightAmount(new BigDecimal(0));//运费金额
        order.setPromotionAmount(new BigDecimal(0));
        order.setPromotionInfo("欢迎购买");
        // 订单的总金额，可获得的积分，积分抵扣金额，应付金额
        ...
        // 使用rocketmq发送异步下单，用户的id，订单，订单信息
        OrderMessage orderMessage = new OrderMessage();
        orderMessage.setUserId(memberId);
        ...
        try {
            boolean sendStatus = orderMessageSenderNomal.sendNormalCreateOrderMsg(orderMessage);
            if (sendStatus == false) {
                // 还原原先被扣减的库存
                incrRedisStock(productId);
                // todo 清除本地显示销售完的标记

                return CommonResult.success(result,"下单失败");
            } else {
                // 打上排队标记
        ...
        return CommonResult.failed(result,"正在下单");
    }
    ...
    private boolean preDecrRedisStock(Long productId,Long promotionId) {
        ...
        if (stock < 0) {
            // 在这里进行加以处理
            if (!redisOpsUtil.hasKey(RedisKeyPrefixConst.NORMAL_STOCK_REFRESHED_MESSAGE_PREFIX)) {
                // 同步一下数据，防止预下单太多，但实际上买的少
                if (orderMessageSenderNomal.sendNormalStockSyncMessage(productId,promotionId));
            }
        }
        return true;
    }
...
//    @GlobalTransactional(name = "generateOrder",rollbackFor = Exception.class)
    ...
    public CommonResult generateOrder(OrderParam orderParam,Long memberId) throws BusinessException {
        ...
        for (CartPromotionItem cartPromotionItem : cartPromotionItemList) {
            // 生成订单商品信息 ，商品id,商品名称，商品图片，商品属性，商品品牌
            OmsOrderItem orderItem = new OmsOrderItem();
            ...
            // 商品货码，商品的价格，商品购买数量，商品的skuid。商品sku码
            orderItem.setProductSn(cartPromotionItem.getProductSn());
            orderItem.setProductPrice(cartPromotionItem.getPrice());
            orderItem.setProductQuantity(cartPromotionItem.getQuantity());
            orderItem.setProductSkuId(cartPromotionItem.getProductSkuId());
            orderItem.setProductSkuCode(cartPromotionItem.getProductSkuCode());
            // 商品分类id,商品促销积分，商品促销名称，商品积分，商品成长值
        ...
        // 进行库存锁定 ==>todo 在这里添加分布式事务，这里选择的是强一致性
    ...
    @Override
    public CommonResult getDetailOrder1(Long orderId){
        CommonResult commonResult = CommonResult.success(portalOrderDao.getDetail(orderId));
    ...
    public Integer paySuccess(Long orderId,Integer payType) {
        //修改订单支付状态
        OmsOrder order = new OmsOrder();
        order.setId(orderId - 200);
        order.setStatus(1);
        order.setPayType(payType);
        order.setPaymentTime(new Date());
        orderMapper.updateByPrimaryKeySelective(order);
        // 恢复所有下单商品的锁定库存，扣减真实库存
        OmsOrderDetail orderDetail = portalOrderDao.getDetail(orderId - 200);
